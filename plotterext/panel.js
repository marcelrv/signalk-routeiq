...
init().catch((e) => {
  ...
  $("unsupported").textContent =
    "Failed to connect to the chartplotter: " + (e?.message || e);
});
...
async function init() {
  ...
  $("route-refresh").addEventListener("click", () => {
    checkEngine();
    loadWaypoints();
    refreshRoutes();
  });
  ...
  $("wp-select").addEventListener("change", () => {
    $("btn-wp").disabled = !$("wp-select").value;
  });
  $("poi-go").addEventListener("click", () => run(searchPois));
  $("poi-query").addEventListener("keydown", (e) => {
    if (e.key === "Enter") run(searchPois);
  });
...
function tideEnabled() {
  return tidesAvailable && $("tide-cb").checked;
}
...
async function checkTides() {
  try {
    ...
    const st = await fetch(
      API + `/tides/status?latitude=${pos.latitude}&longitude=${pos.longitude}`,
...
async function checkEngine() {
  ...
  try {
    ...
    if (st.initError) {
      ...
      el.textContent =
        "Routing engine ready (" + (st.filenames || []).join(", ") + ")";
    }
  } catch (e) {
    engineReady = false;
    el.className = "status error";
...
function renderRouteState() {
  ...
  $("btn-revert").disabled =
    busy || !r || !sessions.get(selectedId)?.revertPoints;
...
async function calculate() {
  ...
  const requestPoints =
    session && samePoints(points, session.generated)
  ...
  if (coords.length < 2)
    throw new Error("Routing engine returned no usable geometry");
  ...
  setStatus(
    "route-status",
    "Route updated on the chart — review and Save to keep it.",
    "ok",
  );
...
async function revert() {
  ...
  const points = session.revertPoints.map((p, i) => ({
    ...p,
    name: p.name || "WP" + (i + 1),
  }));
...
async function routeToDestination() {
  ...
  if (!dest) {
    setStatus(
      "route-status",
      "No active destination — set one first (e.g. “Navigate to here” on the chart).",
      "error",
    );
    return;
  }
  ...
  if (cleared) {
    setStatus(
      "route-status",
      "Route created and active destination cleared — review and Save to keep it.",
      "ok",
    );
  }
}
...
async function clearActiveDestination() {
  try {
    const resp = await fetch("/signalk/v2/api/vessels/self/navigation/course", {
      method: "DELETE",
    });
    return resp.ok;
  } catch {
...
async function activeDestination() {
  ...
  try {
    const p = await fetch(
      "/signalk/v1/api/vessels/self/navigation/courseGreatCircle/nextPoint/position/value",
    ).then((r) => (r.ok ? r.json() : null));
    if (typeof p?.latitude === "number" && typeof p?.longitude === "number")
      return p;
  } catch {
...
async function loadWaypoints() {
  try {
    const wps = await fetch("/signalk/v2/api/resources/waypoints").then((r) =>
      r.ok ? r.json() : {},
    );
    const sel = $("wp-select");
    ...
    const entries = Object.entries(wps || {})
      .map(([id, wp]) => {
        ...
        return {
          id,
          name: wp.name || wp?.feature?.properties?.name || "Unnamed",
          lon: c[0],
          lat: c[1],
        };
      })
...
async function searchPois() {
  ...
  for (const poi of data.results) {
    ...
    const color =
      t === "bridge" && props.subtype === "opening"
        ? CROSSING_COLORS.opening
        : POI_COLORS[t] || "#6d96c0";
...
function renderSummary(result) {
  ...
  if (tideActive && typeof result.totalSecondsNoTide === "number") {
    const deltaMin = Math.round(
      (result.totalSeconds - result.totalSecondsNoTide) / 60,
    );
    line +=
      "  ·  tide " + (deltaMin > 0 ? "+" : "−") + Math.abs(deltaMin) + "m";
...
function renderItinerary(itinerary) {
  ...
  let cumSeconds = 0;
  let cumExact = itinerary.every(
    (p, i) =>
      i === itinerary.length - 1 ||
      (p.leg && typeof p.leg.seconds === "number"),
  );
  ...
  for (let i = 0; i < itinerary.length; i++) {
    ...
    if (typeof p.turn === "number" && Math.abs(p.turn) >= 10) {
      ...
      turn.textContent =
        (p.turn > 0 ? "↱ stb " : "↰ port ") + Math.abs(p.turn) + "°";
    ...
    meta.textContent =
      fmtDistance(p.distanceFromStart) +
      (p.distanceFromStart > 0
        ? " · " +
          (cumExact
            ? fmtDurationSec(cumSeconds)
            : fmtDuration(p.distanceFromStart))
        : "");
    const legStartSeconds = cumSeconds;
    if (!isLast && p.leg && typeof p.leg.seconds === "number")
      cumSeconds += p.leg.seconds;
    ...
    if (!isLast && p.leg) {
      ...
      const bits = [];
      if (typeof p.courseToNext === "number")
        bits.push("→ " + String(p.courseToNext).padStart(3, "0") + "°");
      bits.push(fmtDistance(p.leg.distance));
      if (typeof p.leg.seconds === "number")
        bits.push(fmtDurationSec(p.leg.seconds));
      // Part of the leg time above — shown separately so an hour at a lock is
      // not mistaken for slow going.
      if (p.leg.waitSeconds > 0)
        bits.push("incl. ⏳ " + fmtDurationSec(p.leg.waitSeconds) + " wait");
      if (typeof p.leg.currentKn === "number") {
        bits.push(
          "current " +
            (p.leg.currentKn > 0 ? "+" : "") +
            p.leg.currentKn.toFixed(1) +
            " kn",
        );
      }
      if (typeof p.leg.minDepth === "number")
        bits.push("depth ≥ " + p.leg.minDepth.toFixed(1) + " m");
      if (typeof p.leg.maxAirDraft === "number")
        bits.push("air ≤ " + p.leg.maxAirDraft.toFixed(1) + " m");
      if (typeof p.leg.minWidth === "number")
        bits.push("width ≥ " + p.leg.minWidth.toFixed(0) + " m");
      const stats = document.createElement("div");
      stats.innerHTML = bits
        .map((b) => '<span class="constraint">' + b + "</span>")
        .join(" · ");
      ...
      let waitSoFar = 0;
      const legSailSeconds =
        typeof p.leg.seconds === "number"
          ? p.leg.seconds - (p.leg.waitSeconds || 0)
          : null;
      for (const [, entries] of grouped) {
        ...
        const color =
          e0.type === "lock"
            ? CROSSING_COLORS.lock
            : hasFixed
              ? CROSSING_COLORS.fixed
              : CROSSING_COLORS.opening;
        row.appendChild(poiBadge(e0.type, { subtype: e0.subtype }, color));
        if (hasFixed && hasOpening) {
          row.appendChild(
            poiBadge("bridge", { subtype: "opening" }, CROSSING_COLORS.opening),
          );
        }
        ...
        const nameHasSpanInfo = /\((?:opening span|fixed(?:,[^)]*)?)\)/.test(
          e0.name || "",
        );
        let info = "";
        if (nameHasSpanInfo) {
        ...
        if (typeof e0.distanceFromStart === "number") {
          ...
          if (cumExact && legSailSeconds !== null && p.leg.distance > 0) {
            ...
            cdist.textContent += fmtDurationSec(
              legStartSeconds + waitSoFar + legSailSeconds * frac,
            );
          } else {
        ...
        if (waitS > 0) {
          waitSoFar += waitS;
          cdist.textContent +=
            (cdist.textContent ? " · " : "") + "⏳ " + fmtDurationSec(waitS);
        }
        row.append(cn, cdist);
        leg.appendChild(row);
      }
...
async function scanDepartures() {
  const r = selectedRoute();
  if (!r) {
    setStatus("route-status", "Select a route on the chart first.", "error");
    return;
  }
  const list = $("dep-list");
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      hideDepartures();
      throw new Error(
        (data && data.error) || "Departure scan failed (" + resp.status + ")",
      );
    }
    depScanVisible = true;
    ...
    if (!streamed || !resp.body) {
      const data = await resp.json();
      depSteps = (data.departures || []).map((d) => ({
      ...
      await readNdjson(resp, (ev) => {
        if (ev.type === "meta") {
          depSteps = (ev.departureTimes || []).map((t) => ({
            departureTime: t,
            result: null,
          }));
...
function renderDepartures() {
  ...
  const done = depSteps.filter(
    (s) => s.result && typeof s.result.totalSeconds === "number",
  );
  if (!done.length && !depAbort) {
    list.innerHTML =
      '<div class="hint">No departures could be evaluated.</div>';
    return;
  }

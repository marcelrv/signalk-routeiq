...
const STATIONS = [
  { id: 'north', lat: 52.08, lon: 5.00, offset: 0 },
  ...
  { id: 'south', lat: 51.92, lon: 5.00, offset: 12.5 },
  { id: 'east', lat: 52.00, lon: 5.12, offset: -3.25 },
];
...
describe('tide-informed depth', () => {
  describe('riseAt — the height lookup', () => {
    ...
    it('reads zero at low water and the full range at high water', async () => {
      ...
      assert.ok(Math.abs(atLow!) < 0.02, `expected ~0 m at low water, got ${atLow}`);
  ...
  describe('depthAtPassage — folding the rise into a charted depth', () => {
    ...
    it('leaves the depth alone when there is no tide to consult', () => {
      assert.strictEqual(engine.depthAtPassage(1.5, 52, 5, 1000, undefined), 1.5);
      assert.strictEqual(
  ...
  describe('a route that takes a different channel at high water', () => {
    ...
    before(async () => {
      ...
      for (const p of [dbPath, overlayPath]) if (fs.existsSync(p)) fs.unlinkSync(p);
    ...
    function viaDetour(route: { features?: Array<{ geometry: { coordinates: number[][] } }> }): boolean {
      const coords = (route.features ?? []).flatMap((f) => f.geometry.coordinates);
      return coords.some(
        ([lon, lat]) => Math.abs(lat - C.lat) < 1e-6 && Math.abs(lon - C.lon) < 1e-6,
      );
    }

    it('takes the deep way round at low water', async () => {
      const route = await routeAt(lowWaterMs);
      assert.ok(viaDetour(route), 'expected the detour through C while the tide is out');
    });

    it('takes the short shallow channel at high water', async () => {
      const route = await routeAt(highWaterMs);
      assert.ok(!viaDetour(route), 'expected the direct hop once the tide has risen');
      assert.ok(route.totalDistance! < 3000, `expected the ~2396 m direct route, got ${route.totalDistance}`);
    });
    ...
    it('reports the charted depth and the passage depth side by side', async () => {
      ...
      assert.ok(shallow, 'the direct route should still report its charted 1.5 m');
      assert.ok(
        shallow!.minDepthAtPassage! > 2.3,
        `expected the passage depth to clear 2.3 m, got ${shallow!.minDepthAtPassage}`,
      );
      assert.ok(
        Math.abs(shallow!.tideRiseM! - (shallow!.minDepthAtPassage! - 1.5)) < 0.01,
    ...
    it('leaves the route alone when tides are off', async () => {
      ...
      assert.ok(viaDetour(route), 'expected the charted-only detour with tides off');
